...
from coffee_shop.models import Coffee, Cheque
...
def basket_success(request):
    basket = Basket(request)
    desc = ''
    for item in basket.basket.values():
        desc += f"{item['name_prod']} {item['count_prod']} шт. по цене {item['price_prod']} за шт.\n"
    ...
    return render(request, 'basket/success.html', context)
